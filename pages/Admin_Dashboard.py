...
def create_risk_chart(df):
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='markers',
        name='⚠️ Alert',
        marker=dict(color='#e53e3e', size=12, symbol='triangle-up', line=dict(width=1, color='white'))
    ))
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=True, gridcolor='#e2e8f0', gridwidth=1, title=dict(font=dict(color='#4a5568')), tickfont=dict(color='#4a5568')),
        yaxis=dict(title="Risk Score", range=[0, 1], tickformat='.0%', showgrid=True, gridcolor='#e2e8f0', gridwidth=1, title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568')),
...
def create_sentiment_chart(df):
    ...
    fig.update_yaxes(title_text="Sentiment", secondary_y=False, showgrid=True, gridcolor='#e2e8f0', title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568'))
    fig.update_yaxes(title_text="Momentum", secondary_y=True, showgrid=False, title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568'))
...
def create_volume_chart(df):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode='lines+markers',
            marker=dict(size=5, color='#e53e3e')
        ),
        secondary_y=True
    )
    ...
    fig.update_yaxes(title_text="Mentions", secondary_y=False, showgrid=True, gridcolor='#e2e8f0', title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568'))
    fig.update_yaxes(title_text="Panic Index", secondary_y=True, showgrid=False, title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568'))
...
def main():
    ...
    with col2:
        ...
        <div style="text-align: center; padding: 1rem 0;">
            ...
            <p style="color: #4a5568;">Admin Analytics Dashboard</p>
    ...
    with st.sidebar:
        ...
        <div style="text-align: center; margin-bottom: 1rem;">
            ...
            <div style="font-size: 0.7rem; color: #4a5568;">{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if show_models:
        ...
        fig_models = go.Figure(data=[
            go.Bar(
                x=model_names,
                y=model_data,
                marker_color=['#3182ce', '#38a169', '#ecc94b', '#e53e3e'][:len(model_data)],
                text=[f"{x:.1%}" for x in model_data],
                textposition='outside',
                textfont={'color': '#1a1a2e', 'weight': 'bold'},
                width=0.6
            )
        ])
        ...
        fig_models.update_layout(
            ...
            showlegend=False,
            font=dict(family='Inter, sans-serif', size=12, color='#1a1a2e'),
            yaxis=dict(title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568')),
            xaxis=dict(title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568'))
        )
        ...
        st.markdown("""
        <div class="insight-box">
            <strong>🎯 Ensemble Decision:</strong><br>
    ...
    with col1:
        ...
        fig_hist.update_layout(
            ...
            font=dict(family='Inter, sans-serif', size=12, color='#1a1a2e'),
            xaxis=dict(title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568'), tickformat='.0%'),
            yaxis=dict(title_font=dict(color='#4a5568'), tickfont=dict(color='#4a5568'))
        )
    ...
    <div style="text-align: center; color: #718096; font-size: 0.7rem; padding: 1rem;">
        🐻 THE BEAR PATROL | Admin Analytics Dashboard | Shani • Esther • Matthew • Desmond<br>
